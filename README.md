...
A small, privacy-first Chrome extension that searches, navigates, selects, and exports the ChatGPT conversation currently open in your browser to local files or the clipboard. It also keeps a reusable prompt library locally and can insert user-selected text into a new ChatGPT chat.
...
- Processes exports, prompt storage, and selected-text handoff locally. No telemetry, tracking, developer server, or extension account is used.
...
6. Select text on a page or open a ChatGPT conversation, then use the extension button.
...
| `scripting` | Read selected text, read and navigate conversations, insert text into ChatGPT, and access reusable assets after the user requests an action. |
...
The prompt library uses storage belonging to the extension popup and does not add a browser permission. The extension requests no broad mandatory host permissions. The optional ChatGPT origin is requested only when the selected-text handoff is used. ZIP asset capture may re-read file URLs already exposed by the currently open ChatGPT page, using that page's existing session. Captured content is written only into the local ZIP and is not sent to the developer or another service.

## Privacy

See [PRIVACY.md](PRIVACY.md). Conversation content, saved prompts, and selected text stay on the user's device and are not sent to the developer or third parties by the extension.
